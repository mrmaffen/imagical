...
function searchWiki (keyWord, thumbWidth, imgLimit, locale){

	//some pseudo argumentcheck
	if( !(keyWord == "") && thumbWidth > 0 && imgLimit > 0 && imgLimit <= 500){

		var resultArray = new Array();
		var titleQueryURL = "http://" + locale + ".wikipedia.org/w/api.php?action=query&format=json&callback=?&titles=" + keyWord + "&prop=images&imlimit=" + imgLimit + "&redirects";
		...
		$.ajax({
			...
			success: function(data) {
				...
				for( var i=0; i < titleArray.length; i++){
					var urlQueryURL = "http://" + locale + ".wikipedia.org/w/api.php?action=query&redirects&format=json&callback=?&titles=" + titleArray[i] + "&prop=imageinfo&iiprop=url&iiurlwidth=" + thumbWidth;
					$.ajax({
						url: urlQueryURL,
						dataType: "jsonp",
